...
for f in ssl2 ssl3 tls1
do  
	for p in 8773 8443
	do  
    		echo -e "======= CHECKING $p WITH $f ======="; 
    		echo -e 'GET /\n\n' | openssl s_client -connect 127.0.0.1:$p -quiet -$f 2>&1 >/dev/null | xargs -i echo "127.0.0.1:$p openssl $f {}" 
    	done
done  
...
do
	...
	gnutls-cli-debug -p $p 127.0.0.1  | xargs -i echo "127.0.0.1:$p gnutls {}"
done
